...
internal class INSTRUCTION_DECODER(Int instruction)
{
    ...
    public static implicit operator(Byte Mode, Byte OpCode, Byte Destination, Byte A, Byte B, Bit IsImm, Short ImmVal)(INSTRUCTION_DECODER decoder)
    {
        ...
        Byte b = ((Int)new SINDEXER<Int>(decoder._instruction, 0)).Into<Byte>();
        Bit isImm = ((Int)new SINDEXER<Int>(decoder._instruction, -3)).Into<Bit>();
        Short immVal = ((Int)new SINDEXER<Int>(decoder._instruction, -16)).Into<Short>();
