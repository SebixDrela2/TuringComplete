...
public class COUNTER4<T> : IStateGate<T> where T : struct, IValue<T>
{
    ...
    public COUNTER4(T initialValue, CLOCK clock)
    {
        _clock = clock;
        _register = new REGISTER<T>(_clock);
        _one = new CONST<T>(4);
    ...
    public static implicit operator T(COUNTER4<T> counter)
    {
